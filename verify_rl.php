...
$db = new Medoo([
	...
	'database_file' => 'verify_by_do_rl.sqlite'
]);
...
function verify(){
	...
	global $db;
	$record=$db->get('rl_emails',['email','id'],[
		'valid'=>null,
		]);
	$left=$db->count('rl_emails',[
		'valid'=>null,
		]);
	...
	echo ' | '.$result['id'].' '.$resultText.' at '.date('Y-m-d H:i:s').' emails left to do: '.$left."\n";
	$db->update('rl_emails',['valid'=>$resultText],[
			'id'=>$record['id'],
			]);
